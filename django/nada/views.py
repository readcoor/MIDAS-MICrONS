...
def filtered_query(cls, collection, experiment, channel,
                   x_start, y_start, z_start, x_stop, y_stop, z_stop,
                   id=None):
    '''
    Returns a query to find objects (of class==cls) within a bounded volume.
    Filter for only objects belonging to the given collection/experiment/channel.
    If ID is provided, also filter for only objects with that name. 
    '''
    table_name = cls._meta.db_table

    # escape for sql injection
    (collection_e, experiment_e, channel_e) = (esc(collection), esc(experiment), esc(channel))
    ...
    query = 'SELECT n.id FROM %s n, nada_experiment e, nada_collection c, nada_channel l' % table_name
    query += ' where n.keypoint &&& %s' %  box
    query += ' and c.id = e.collection_id'
    query += ' and e.id = n.experiment_id'
    query += ' and l.id = n.channel_id'
    query += ' and c.name = %s' % collection_e
    query += ' and e.name = %s' % experiment_e
    query += ' and l.name = %s' % channel_e
    if id:
        query += " and n.name = '%s'" % id
    query += ';'
    return cls.objects.raw(query)

def get_filtered_ids(cls, collection, experiment, channel,
                     x_start, y_start, z_start, x_stop, y_stop, z_stop):
    '''
    Returns a list of ids for objects (neuron or synapse) in the given location.
    '''
    return [obj.name for obj in \
            filtered_query(cls, collection, experiment, channel,
                           x_start, y_start, z_start, x_stop, y_stop, z_stop)]

def get_filtered_object(cls, collection, experiment, channel,
    ...
    if len(objs)==0:
        err_msg = "Not found: %s with collection=%s, experiment=%s, channel=%s, id=%s" % \
                  (cls.__name__, collection, experiment, channel, id)
        return Response(err_msg, status.HTTP_400_BAD_REQUEST)
    if len(objs)>1:
        err_msg = "Not unique, found %s objects with collection=%s, experiment=%s, channel=%s, id=%s" % \
                  (len(objs), collection, experiment, channel, id)
...
# S1
@api_view(['GET'])
def is_synapse(request, collection, experiment, channel, id):
    synapse = Synapse.get_obj_or_400(collection, experiment, channel, id)
    if isinstance(synapse, Synapse):
        found = True
    else:
        neuron = Neuron.get_obj_or_400(collection, experiment, channel, id)
...
# S2
@api_view(['GET'])
def synapse_ids(request, collection, experiment, channel, resolution,
                           x_start, x_stop, y_start, y_stop, z_start, z_stop):
    ids = get_filtered_ids(Synapse, collection, experiment, channel,
                           x_start, y_start, z_start, x_stop, y_stop, z_stop)
    result = { "ids": ids }
    return Response(result, status=status.HTTP_200_OK)
...
# S3 
@api_view(['GET'])
def synapse_keypoint(request, collection, experiment, channel, resolution, id):
    synapse = Synapse.get_obj_or_400(collection, experiment, channel, id)
...
# S4
@api_view(['GET'])
def synapse_parent(request, collection, experiment, channel, id):
    synapse = Synapse.get_obj_or_400(collection, experiment, channel, id)
    if isinstance(synapse, Response):
        return synapse  # return HTTP_400 response
    neurons = {}
...
# S5
@api_view(['GET'])
def is_neuron(request, collection, experiment, channel, id):
    neuron = Neuron.get_obj_or_400(collection, experiment, channel, id)
    if isinstance(neuron, Neuron):
        found = True
    else:
...
# S6
@api_view(['GET'])
def neuron_ids(request, collection, experiment, channel, resolution,
                           x_start, x_stop, y_start, y_stop, z_start, z_stop):
    ids = get_filtered_ids(Neuron, collection, experiment, channel,
                           x_start, y_start, z_start, x_stop, y_stop, z_stop)
    result = { "ids": ids }
    return Response(result, status=status.HTTP_200_OK)
...
# S7
@api_view(['GET'])
def neuron_keypoint(request, collection, experiment, channel, resolution, id):
    neuron = Neuron.get_obj_or_400(collection, experiment, channel, id)
...
# S8
@api_view(['GET'])
def neuron_children(request, collection, experiment, channel,  resolution,
                        x_start, x_stop, y_start, y_stop, z_start, z_stop, id):
    neuron = get_filtered_object(Neuron, collection, experiment, channel,
...
# S9
@api_view(['GET'])
def voxel_list(request, collection, experiment, channel,  resolution,
                     x_start, x_stop, y_start, y_stop, z_start, z_stop, id):
    neuron = get_filtered_object(Neuron, collection, experiment, channel,
                                 x_start, y_start, z_start, x_stop, y_stop, z_stop, id=id)
    if isinstance(neuron, Response):
...
#@api_view(['GET'])
#def voxel_list_remote(request, collection, experiment, channel,  resolution,
#               x_start, x_stop,
#               y_start, y_stop,
#               z_start, z_stop, id):
...
#    result = boss.get_channel(channel, collection, experiment)
#    return Response(result.raw, status=status.HTTP_200_OK)


# S10
@api_view(['GET'])
def synapse_compartment(request, collection, experiment, channel,  id):
    synapse = Synapse.get_obj_or_400(collection, experiment, channel, id)
    if isinstance(synapse, Response):
        return synapse  # return HTTP_400 response
...
# S11
@api_view(['GET'])
def neuron_celltype(request, collection, experiment, channel,  id):
    neuron= Neuron.get_obj_or_400(collection, experiment, channel, id)
    if isinstance(neuron, Response):
        return neuron  # return HTTP_400 response
...
# S12
@api_view(['GET'])
def neuron_stimulus(request, collection, experiment, channel,  id, start_time, end_time):
    neuron = Neuron.get_obj_or_400(collection, experiment, channel, id)
    if isinstance(neuron, Response):
        return neuron  # return HTTP_400 response
...
# S13
@api_view(['GET'])
def neuron_activity(request, collection, experiment, channel, id, start_time, end_time):
    neuron = Neuron.get_obj_or_400(collection, experiment, channel, id)
    if isinstance(neuron, Response):
        return neuron  # return HTTP_400 response
